...
**Always run `./vendor/bin/catraca` first.** It is the single source of truth for the quality gate and will report any style, static analysis, duplication, or test failures in one pass. Only fall back to individual commands if you need focused output.

| Task | Command |
|------|---------|
| Check all problems and fix | `./vendor/bin/catraca` |
| Run all tests | `composer test` (alias for `./vendor/bin/pest`) |
| Run unit tests only | `composer test:unit` |
| Run tests with coverage | `composer test:coverage` |
| Static analysis | `composer analyse` (PHPStan level 6 on `src/`) |
| Code style fix | `./vendor/bin/pint` |

Prefer `composer` scripts over `make` targets — the Makefile references non-existent scripts (`test:types`, `test:type-coverage`, `test-plugin.sh`) that are not defined in `composer.json`.

There is **no** `composer test:types`, `test:type-coverage`, or Pint script. Pint is installed but has no custom `pint.json` — it uses the default Laravel preset.

## Architecture

Single PHP 8.3+ library under `B7s\FluentCut\`. Key components:

- **`src/FluentCut.php`** — fluent builder entrypoint. Holds clips, transitions, audio tracks, and delegates to `CompositorService` on `render()`.
- **`src/Services/CompositorService.php`** — orchestrates segment rendering, transition passes, audio mixing, and final muxing.
- **`src/Services/FFmpegService.php`** — wraps `ffmpeg`/`ffprobe` binary calls, probing, and GPU encoder detection.
- **`src/Services/SegmentCommandBuilder.php`** — constructs per-segment FFmpeg filter graphs.
- **`src/Config.php`** — loads `fluentcut-config.php` from project root → cwd → package root, falling back to hard-coded defaults.

## Testing

- Pest PHP with a single `Unit` testsuite (`phpunit.xml`).
- `tests/Pest.php` bootstraps `TestCase` for the `Unit` directory.
- Tests in `tests/Unit/` do **not** invoke real FFmpeg; they verify builder state, enum logic, and filter string generation.

## CLI

`bin/fluentcut` exposes two Symfony Console commands:
- `doctor` — verify PHP 8.3+, FFmpeg, and FFprobe availability.
- `info <file>` — probe a media file and display metadata.

## Configuration

The repo contains a root `fluentcut-config.php` that sets production-oriented defaults (e.g., `timeout => 14400`). This file is **not** the package fallback — the package fallback lives in `Config::defaults()`. When modifying config loading logic, verify both paths.

## Release Workflow

- `make release version=x.y.z` runs the full test suite, commits any pending changes, creates and pushes a Git tag, and updates the `version` file.
- CI (`.github/workflows/release.yml`) triggers only on `v*` tags. It installs deps, runs `./vendor/bin/pest`, then creates a GitHub release with auto-generated notes.
- The `version` file is updated **after** the tag is pushed via a second commit.

## Quality Gate

`b7s/catraca` is a dev dependency. `catraca_baseline.json` exists locally (currently untracked) and encodes targets: 85% coverage, 0 PHPStan errors, 0 style violations, max 2% duplication. If you modify code that affects these metrics, update or acknowledge the baseline.

## Notes

- `composer.lock` is listed in `.gitignore` and is **not** tracked in git.
- The library depends only on Symfony Console and Process — there is no Laravel framework dependency despite the Laravel-inspired API design.
